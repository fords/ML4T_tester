...
class DTLearner(object):
    ...
    def make_DT_not_forest(self, Dataset ):
        ...
        corrs_array = abs(np.array([np.corrcoef(x_attr, y_train.T) for x_attr in x_train.T]))
        corrs_matrix = np.zeros((x_train.shape[1], 0))
        for index, corr in enumerate(corrs_array):
            if(math.isnan(corr[0, 1])):
                corrs_matrix = np.append(corrs_matrix, np.array([0]))
            else:
                corrs_matrix = np.append(corrs_matrix, np.array(corr[0,1]))
        length = np.argmax(corrs_matrix)
...
if __name__=="__main__":
    print "the secret clue is DTLearner\n"

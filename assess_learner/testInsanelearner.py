...
if __name__ == '__main__':
    ...
    #learner = il.InsaneLearner(baglearner =bl.BagLearner, learner = dt.DTLearner, kwargs =  {"leaf_size":1} ,bags = 10, boost = False, verbose = False)
...
#
    ...
    for size in range(1, maxLeafSize+1):
        #learner = bl.BagLearner(baglearner =bl.BagLearner, learner =dt.DTLearner,  kwargs =  {"leaf_size":1}, bags = 10, boost = False, verbose = False)# create learner
        learner = il.InsaneLearner(verbose = False)
        for i in range(10):
            #np.random.shuffle(data) # shuffle in-place
